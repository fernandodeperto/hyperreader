...
//   1. seed pages via the API (POST /api/pages)
//   2. the table renders them (name/description)
//   3. search filters the table (real FTS5, GET /api/pages?q=)
//   4. clicking a row opens that page's raw rendered HTML in a NEW
//      BROWSER TAB (window.open to GET /api/pages/{slug}/content), with
//      zero app chrome — the page's inline <script> runs and leaves a
...
// so an inline <script> in the page executes natively — its marker
...
// uniquely-slugged page so it is robust to whatever test 1 left behind.
import { test, expect, type Page, type Locator } from "@playwright/test";

// A page whose HTML contains an inline <script> that writes a marker
...
// A uniquely-slugged page used by test 2 so it can search-isolate its own
// row regardless of what test 1 seeded into the shared store.
const PAGE_UNSANDBOXED_PROOF = {
  slug: "unsandboxed-proof-page",
  name: "Unsandboxed Proof Page",
...
// A tall-content page used by test 3 to prove the new browser tab renders
// the FULL page with browser-native scrolling (M002 / Branch B): there is
// no in-app iframe to cap or measure, so the new tab's document is the sole
...
const PAGE_TALL_CONTENT = {
  slug: "tall-content-scroll-page",
  name: "Tall Content Scroll Page",
  description: "proves the new tab renders the full tall page",
  html:
...
// seed POSTs a page via the real API and returns its slug. The table view
// is populated by GET /api/pages, so seeding through the API proves the UI
// reflects real storage — not test-injected DOM.
async function seed(page: Page, doc: typeof PAGE_WITH_INLINE_SCRIPT): Promise<string> {
  const res = await page.request.post("/api/pages", { data: doc });
...
test("primary user loop: table -> search -> unsandboxed new tab -> table intact", async ({ page }) => {
  // 1. Seed two real pages through the API.
  await seed(page, PAGE_WITH_INLINE_SCRIPT);
  await seed(page, PAGE_OTHER);
  ...
  // most-recently-changed-first: the second-seeded page is on top
  ...
  //    browser tab (window.open to /api/pages/{slug}/content).
  const deployRow = page.locator("#pages-table tbody tr", { hasText: "Deploy Runbook" });
  ...
  // The static marker proves the page's HTML rendered in the new tab.
  ...
  await popup.close();
  await expect(rows).toHaveCount(2);
  await expect(page.locator("#pages-table tbody tr", { hasText: "Deploy Runbook" })).toBeVisible();
...
test("new tab renders page HTML unsandboxed via search-isolated row", async ({ page }) => {
  ...
  // its own uniquely-named page so it is robust to whatever test 1 left in
  // the shared store (the webServer runs once for the whole suite).
  await seed(page, PAGE_UNSANDBOXED_PROOF);

  await page.goto("/");
  await expect(page.locator("#loading-state")).toBeHidden();

  // Isolate this test's page via the real FTS5 search so cross-test rows do
  // not affect the click target.
  await page.locator("#search").fill("Unsandboxed Proof");
  const rows = tableRows(page);
  await expect(rows).toHaveCount(1);
  await expect(rows.nth(0)).toContainText("Unsandboxed Proof Page");
...
test("new tab renders the full tall page with browser-native scroll (M002)", async ({ page }) => {
  // Proves Branch B's full-page render: with the in-app iframe gone, a
  // tall page opens in a new top-level tab whose document is the sole
  // scroll surface. There is no iframe to cap or auto-resize; the browser
  // renders the complete 2000px content block and scrolls it natively.
  // Search-isolates its own tall page so it is robust to whatever tests 1
  // and 2 left in the shared store.
  await seed(page, PAGE_TALL_CONTENT);

  await page.goto("/");
  await expect(page.locator("#loading-state")).toBeHidden();

  // Isolate this test's page via the real FTS5 search.
  await page.locator("#search").fill("Tall Content Scroll");
  const rows = tableRows(page);
  await expect(rows).toHaveCount(1);
  await expect(rows.nth(0)).toContainText("Tall Content Scroll Page");
  ...
  // Wait for the page's HTML to render in the new tab — the static marker
  // proves content loaded (and fails loudly on an empty page rather than
  // passing on a blank tab).
  await expect(popup.locator("#static-marker")).toHaveText("static");
